...
test("consolidates parallel tools in source order with wall and work durations", () => {
	...
	assert.equal(rendered.match(/tok —/g)?.length, 3);
});

test("keeps failures and model-backed usage on their batch members", () => {
	const tracker = new TimingTracker();
	beginTurn(tracker);
	tracker.finishAssistant({ stopReason: "toolUse", content: [] }, at(1_050, 60));
	...
	tracker.consumeToolResult("child", usage(7_700, 512, 0, 0, 0.027), false);
	const batch = tracker.finishTurn(0)!;
	const rendered = formatTimingRecord(batch, false).join("\n");

	assert.match(rendered, /read · 100ms · failed · tok —/);
	assert.match(rendered, /subagent · 3\.40s · Σ8\.2k ↑7\.7k ↓512 R0 W0 · \$0\.027/);
});
...
test("keeps a single tool as one compact tool record", () => {
	...
	assert.match(formatTimingRecord(record, false)[0]!, /└ bash .* 500ms · tok —/);
});
...
test("coerces legacy V1 records and preserves unavailable usage", () => {
	...
	assert.match(formatTimingRecord(legacy!, false).join("\n"), /tok —/);
});
